#!/usr/bin/env python3
"""Launch the isolated DSpark confidence-drop rejection experiment."""
...
# Shared with the preceding conditional-confidence experiment so concurrently
# launched observation jobs recognize one another's short pre-spawn leases.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Measure whether a drop from the same-round prior confidence mean "
            "predicts the first rejected DSpark draft position."
        )
    )
...
def write_markdown_header_exclusive(
    ...
    *,
    ...
    header = f"""# DSpark Confidence-Drop Rejection Results
    ...
    for _ in range(attempts):
        ...
        with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
            json.dump(
                {
                    "pid": os.getpid(),
                    "experiment": "confidence_drop_rejection",
...
def main() -> None:
    ...
    markdown_results_path = run_dir / "confidence_drop_results.md"
    progress_dir = run_dir / "progress"
    tensorboard_dir = run_dir / "tensorboard"
    baseline_artifact_dir = tensorboard_dir / "artifacts" / f"step_{cli.step}"
    observation_artifact_root = run_dir / "observations" / "confidence_drop_rejection"

    settings = {
        "schema_version": 1,
        "experiment": "dspark_qwen3_8b_confidence_drop_rejection_prediction",
        "created_at": now_iso(),
        "run_dir": str(run_dir),
        "purpose": {
            "question": (
                "When C_i drops sufficiently below the mean raw conditional "
                "confidence of earlier tokens in the same draft round, is position i "
                "the first rejected position, or is it an accepted false positive?"
            ),
            "conditional_confidence": "C_i = sigmoid(confidence_logits[i]); not cumulative",
            "prior_mean": "C_i_mean = mean(C_0, ..., C_{i-1}), i >= 1",
            "absolute_drop": "C_i_mean - C_i",
            "percentage_drop": "max(0, 1 - C_i / C_i_mean)",
        ...
        "outputs": {
            ...
            "confidence_drop_artifact_root": str(observation_artifact_root),
        },
    }
    ...
    try:
        ...
        from observations.confidence_drop_rejection.confidence_drop_observation import (
            evaluation_worker,
        )
