#!/usr/bin/env python3
"""Summarize a confidence-drop run and optionally backfill Markdown macro averages."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "end of confidence_drop_results.md from dataset_results.jsonl."
        ),
    )
...
def main() -> None:
    ...
    if cli.refresh_markdown_macro:
        ...
        for result in result_rows:
            ...
            summary = result.get("confidence_drop_observation_summary")
            if not isinstance(summary, dict):
                raise ValueError(f"Missing confidence-drop summary for dataset {dataset!r}")
            summaries[dataset] = summary
        if not summaries:
            raise ValueError("No completed dataset summaries are available for macro averaging")
        markdown_path = run_dir / "confidence_drop_results.md"
        refresh_macro_only(
            path=markdown_path,
            summaries=summaries,
            family_specs=(
    ...
    denominator_rows = []
    for result in result_rows:
        summary = result.get("confidence_drop_observation_summary") or {}
