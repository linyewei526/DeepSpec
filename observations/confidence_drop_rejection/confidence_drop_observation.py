"""Evaluate whether a within-round confidence drop predicts draft rejection.

For draft position i >= 1, this experiment compares the raw conditional
confidence ``C_i = sigmoid(confidence_logits[i])`` with the mean confidence of
positions 0..i-1 from the same draft forward pass.  It never uses a cumulative
confidence product and never changes decoding decisions or random state.
"""
...
class ConfidenceDropMetrics:
    ...
    def __init__(
        ...
        *,
        ...
        # [accepted, rejected] counts for which C_i_mean is numerically zero.
    ...
    def observe(
        ...
        *,
        ...
        if confidence_logits is None:
            raise RuntimeError("The confidence-drop experiment requires a confidence head")
        ...
        prior_means = confidences.cumsum(dim=0)[:-1] / torch.arange(
            1,
            proposal_length,
            dtype=confidences.dtype,
            device=confidences.device,
        )
        current_confidences = confidences[1:]
        absolute_drops = prior_means - current_confidences
        percentage_valid = prior_means > 0.0
        safe_prior_means = prior_means.clamp_min(torch.finfo(prior_means.dtype).tiny)
        percentage_drops = (
            1.0 - current_confidences / safe_prior_means
        ...
        return {
            ...
            "definitions": {
                "conditional_confidence": (
                    "C_i = sigmoid(confidence_logits[i]); position-i acceptance "
                    "confidence conditional on the preceding draft prefix; not cumprod"
                ),
                "prior_mean": "C_i_mean = mean(C_0, ..., C_{i-1}); only defined for i >= 1",
                "absolute_drop": "C_i_mean - C_i",
                "percentage_drop": "max(0, 1 - C_i / C_i_mean); undefined when C_i_mean == 0",
...
def _plot_threshold_curves(dataset_dir: Path, report: dict) -> Path:
    ...
    fig.suptitle(f"{report['dataset']} confidence-drop rejection prediction")
    fig.tight_layout()
...
class ConfidenceDropRecorder:
    def __init__(
        ...
        *,
        ...
        self.step = step
        self.current: ConfidenceDropMetrics | None = None
        self.rows: list[dict] = []

    def start(self) -> None:
        if self.current is not None:
            raise RuntimeError("Previous confidence-drop dataset was not finished")
        self.current = ConfidenceDropMetrics(
            device=self.device,
    ...
    def observe(
        ...
        *,
        ...
        if self.current is None:
            raise RuntimeError("ConfidenceDropRecorder.start() was not called")
        ...
        *,
        ...
        if rank == 0:
            ...
            output_payload = {
                ...
                "confidence_drop_observation": report,
                "confidence_drop_observation_summary": summary,
            }
            ...
            print(
                "Confidence-drop observation counts: "
                + json.dumps(
            ...
            print(f"Wrote confidence-drop artifacts to {dataset_dir}", flush=True)
        dist.barrier()
        return summary
                ...
                for row in report[family_key]["thresholds"]:
                    ...
                    for metric in (
                        ...
                        if value is not None:
                            writer.add_scalar(
                                f"confidence_drop/{dataset}/{family_key}/{metric}_{threshold_label}",
                                float(value),
                                self.step,
                            )
        writer.close()
    ...
    def print_results(self) -> None:
        ...
        print("Confidence-drop rejection-prediction denominators:", flush=True)
        print(table.get_string(), flush=True)


class ConfidenceDropEvaluator(Qwen3DSparkEvaluator):
    """Qwen3 DSpark evaluator with isolated confidence-drop counters."""
    ...
    def __init__(self, local_rank: int, args):
        ...
        self.confidence_drop_recorder = ConfidenceDropRecorder(
            device=self.device,
    ...
    def _post_verify(self, proposal, verification) -> None:
        ...
        self.confidence_drop_recorder.observe(
            proposal=proposal,
    ...
    def record_dataset_metrics(self, *, dataset_name: str, metric_summary: dict):
        self.mark_dataset_phase(
            dataset_name,
            "reducing_and_writing_confidence_drop_observations",
        )
        summary = self.confidence_drop_recorder.finish(
            dataset_name=dataset_name,
        ...
        if summary is not None:
            self.confidence_drop_summaries[dataset_name] = summary
    ...
    def record_incremental_dataset_result(
        ...
        *,
        ...
        result = {
            ...
            "confidence_drop_observation_summary": (
                self.confidence_drop_summaries.get(dataset_name)
            ),
        }
        markdown_value = getattr(self.args, "markdown_results_path", None)
        markdown_summary = self.confidence_drop_summaries.get(dataset_name)
    ...
    def log_tensorboard(self) -> None:
        ...
        self.confidence_drop_recorder.log_tensorboard()

    def print_results(self) -> None:
        super().print_results()
        self.confidence_drop_recorder.print_results()
...
def evaluation_worker(local_rank: int, args) -> None:
    ...
    evaluator = ConfidenceDropEvaluator(local_rank, args)
    try:
        evaluator.evaluate()
    finally:
        evaluator.clean_up()
