#!/usr/bin/env python3
"""Launch the isolated DSpark selected-token Markov draft-probability observation experiment."""
...
# Shared by all isolated observation launchers so concurrent jobs cannot lease
# the same automatically selected distributed port.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run DSpark while observing raw selected-token Markov draft probability and the "
...
def main() -> None:
    ...
    markdown_results_path = run_dir / "markov_draft_probability_rejection_thresholds.md"
    ...
    settings = {
        ...
        "purpose": {
            ...
            "rejected_distribution": (
                "P_k of the erroneous submitted token at each first rejected/replaced position."
            ),
            "signed_gap": (
            ...
            "direct_rejection_prediction": (
                ...
                "softmax(markov_corrected_logits[k])[submitted_token] < t (strict). "
                "Position 0 is included; positions after the first rejection or discarded "
                "after accepted EOS are excluded."
            ),
        },
        ...
        "outputs": {
            ...
            "markov_draft_probability_artifact_root": str(observation_artifact_root),
        },
    }
    ...
    try:
        ...
        if str(REPO_ROOT) not in sys.path:
            sys.path.insert(0, str(REPO_ROOT))
        from observations.markov_draft_probability.markov_probability_observation import evaluation_worker
