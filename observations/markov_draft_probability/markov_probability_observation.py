...
class DatasetObservation:
    ...
    def observe(
        ...
        if accepted_count:
            accepted_hits = (
                selected_probabilities[:accepted_count, None]
        ...
        rejected_hits = (
            selected_probabilities[accepted_count] < self.score_threshold_tensor
        ).tolist()
    ...
    def to_payload(self) -> dict:
        return {
            ...
            "direct_rejection_prediction": {
                "comparison": "selected_markov_probability < threshold (strict)",
    ...
    def from_payload(cls, payload: dict) -> "DatasetObservation":
        ...
        if not isinstance(prediction, dict) or not prediction.get("thresholds"):
            raise ValueError(
                ...
                "schema-v3 and earlier results must be rerun for width-0.02 thresholds"
            )
    ...
    def build_report(self, *, dataset_name: str, sample_count: int) -> dict:
        ...
        eligible_accepted = self.accepted_probability.count
        eligible_rejected = self.rejected_probability.count
        ...
        return {
            ...
            "definitions": {
                ...
                "direct_rejection_prediction": (
                    "flag an evaluable token iff its temperature-independent selected-token "
                    "diagnostic Markov probability is strictly below the configured threshold; "
                    "position 0 is included"
                ),
        ...
        *,
        ...
        if markdown_value is not None and markdown_summary is not None:
            ...
            append_dataset_and_refresh_macro(
                ...
                family_specs=((
                    "selected_markov_probability < threshold",
                    "threshold",
                    "thresholds",
                ),),
            )
