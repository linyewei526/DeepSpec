#!/usr/bin/env python3
"""Launch the isolated DSpark selected-token Markov probability-drop rejection experiment."""
...
# Shared by all isolated observation launchers so concurrently launched jobs
# recognize one another's short pre-spawn leases.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Measure whether a drop from the same-round prior selected-token "
            "Markov draft-probability mean predicts the first rejected position."
        )
    )
    ...
    parser.add_argument("--abs-drop-min", default="0.05")
    parser.add_argument("--abs-drop-max", default="0.25")
    parser.add_argument("--pct-drop-min", default="0.05")
    parser.add_argument("--pct-drop-max", default="0.30")
    parser.add_argument("--drop-step", default="0.005")
    parser.add_argument(
        "--max-samples",
        type=int,
        default=None,
...
def write_markdown_header_exclusive(
    ...
    *,
    ...
    header = f"""# DSpark Markov Probability-Drop Rejection Results

- Created at: `{created_at}`
- Settings: `{settings_path}`
- `accepted_share`, `rejected_share / precision`, `accepted_FPR`, `rejection_recall` and `flag_rate` are rendered as percentages.
- A dataset section is appended only after its distributed metrics and artifacts have been reduced successfully.
...
def build_tasks(dataset: str, max_samples: int | None) -> list[tuple[str, int]]:
    selected = (
        list(DATASET_CAPS.items())
        if dataset == "all"
        else [(dataset, DATASET_CAPS[dataset])]
    )
...
def build_decimal_grid(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"Invalid decimal threshold for {family}") from exc
    if not minimum.is_finite() or not maximum.is_finite() or not step.is_finite():
        raise ValueError(f"{family} threshold values must be finite")
    if minimum < 0 or maximum > 1 or maximum < minimum:
        raise ValueError(f"{family} threshold range must satisfy 0 <= min <= max <= 1")
    if step <= 0:
        raise ValueError("--drop-step must be positive")
    span_steps = (maximum - minimum) / step
    integral_steps = span_steps.to_integral_value()
    if span_steps != integral_steps:
        raise ValueError(
            f"{family} range [{minimum}, {maximum}] is not exactly divisible "
            f"by step {step}"
        )
...
def reserve_master_port(
    ...
    """Reserve a free port across concurrent observation launches."""
    ...
    for _ in range(attempts):
        ...
        if not _port_is_bindable(master_addr, port):
            if requested_port is not None:
                raise RuntimeError(
                    f"Requested distributed port is already in use: {master_addr}:{port}"
                )
            continue
        ...
        try:
            ...
            if requested_port is not None:
                raise RuntimeError(
                    f"Requested distributed port is leased by another observation run: {port}"
                )
            continue
        with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
            json.dump(
                {
                    "pid": os.getpid(),
                    "experiment": "markov_probability_drop_rejection",
        ...
        return (
            port,
            lease_path,
            "explicit" if requested_port is not None else "auto_reserved",
        )
...
def validate_run_directory(run_dir: Path) -> None:
    ...
    unexpected = sorted(
        path.name for path in run_dir.iterdir() if path.name != "eval.log"
    )
    if unexpected:
        raise FileExistsError(
            f"Refusing to reuse a non-empty experiment directory {run_dir}; "
            f"found {unexpected}"
        )
...
def main() -> None:
    ...
    absolute_grid = build_decimal_grid(
        cli.abs_drop_min,
        cli.abs_drop_max,
        cli.drop_step,
        family="absolute drop",
    )
    percentage_grid = build_decimal_grid(
        cli.pct_drop_min,
        cli.pct_drop_max,
        cli.drop_step,
        family="percentage drop",
    )
    ...
    if cli.confidence_threshold > 0.0 and not bool(
        draft_config.get("enable_confidence_head", False)
    ):
        raise ValueError(
            "--confidence-threshold > 0 requires a checkpoint confidence head"
        )
    ...
    if cuda_device_count < 1:
        raise RuntimeError(
            "No visible CUDA device; check CUDA_VISIBLE_DEVICES and PyTorch CUDA support"
        )
    ...
    markdown_results_path = run_dir / "markov_probability_drop_results.md"
    progress_dir = run_dir / "progress"
    tensorboard_dir = run_dir / "tensorboard"
    baseline_artifact_dir = tensorboard_dir / "artifacts" / f"step_{cli.step}"
    observation_artifact_root = run_dir / "observations" / "markov_probability_drop_rejection"

    settings = {
        "schema_version": 2,
        "experiment": "dspark_qwen3_8b_markov_probability_drop_rejection_prediction",
        "created_at": now_iso(),
        "run_dir": str(run_dir),
        "purpose": {
            "question": (
                "When P_i drops sufficiently below the mean selected-token Markov "
                "draft probability of earlier tokens in the same draft round, is "
                "position i the first rejected position, or an accepted false positive?"
            ),
            "selected_token_probability": (
                "P_i = q_obs_i[z_i], where z_i is the submitted draft token and "
                "q_obs_i = softmax(markov_corrected_logits_i) without temperature scaling."
            ),
            "semantic_note": (
            ...
            "operational_distribution": (
                ...
                "The observation-only q_obs_i never replaces it."
            ),
            "trajectory_note": (
                "The softmax mapping used by q_obs_i is temperature-independent, but changing "
                "temperature can change sampled prefix tokens and therefore later corrected logits."
            ),
            "prior_mean": "P_i_mean = mean(P_0, ..., P_{i-1}), i >= 1",
            "absolute_drop": "P_i_mean - P_i",
            "percentage_drop": "max(0, 1 - P_i / P_i_mean)",
            "comparison": "drop >= threshold (inclusive)",
            "censoring": (
                "position 0, positions after the first rejection, and positions "
                "after an accepted EOS do not receive accepted/rejected outcomes"
            ),
        },
        "thresholds": {
            "absolute_drop": {
                "minimum": str(absolute_grid[0]),
                "maximum": str(absolute_grid[-1]),
                "step": str(Decimal(cli.drop_step)),
                "count": len(absolute_grid),
                "values": [str(value) for value in absolute_grid],
            },
            "percentage_drop": {
                "minimum": str(percentage_grid[0]),
                "maximum": str(percentage_grid[-1]),
                "step": str(Decimal(cli.drop_step)),
                "count": len(percentage_grid),
                "values": [str(value) for value in percentage_grid],
            },
        },
        "scope": {
        ...
        "models": {
            ...
            "draft_confidence_head_with_markov": draft_config.get(
        ...
        "distributed": {
            "strategy": (
                "one target+draft replica per visible GPU; dataset samples are rank-sharded"
            ),
        ...
        "outputs": {
            ...
            "markdown_results": str(markdown_results_path),
            "progress_dir": str(progress_dir),
            "tensorboard_dir": str(tensorboard_dir),
            "existing_confidence_artifact_dir": str(baseline_artifact_dir),
            "markov_probability_drop_artifact_root": str(observation_artifact_root),
        },
    }

    # The immutable settings file is the first experiment metadata written and
    # is never rewritten by this launcher.
    try:
    ...
    try:
        ...
        print(f"Incremental Markdown results: {markdown_results_path}", flush=True)
        print(f"Reserved distributed endpoint: {cli.master_addr}:{master_port}", flush=True)
        print(
            f"Threshold counts: absolute={len(absolute_grid)}, "
            f"percentage={len(percentage_grid)}",
            flush=True,
        )
        ...
        worker_args = SimpleNamespace(
            ...
            temperature=cli.temperature,
            confidence_threshold=cli.confidence_threshold,
            absolute_drop_thresholds=[float(value) for value in absolute_grid],
            percentage_drop_thresholds=[float(value) for value in percentage_grid],
            tensorboard_dir=str(tensorboard_dir),
        ...
        from observations.markov_probability_drop_rejection.markov_probability_drop_observation import (
            evaluation_worker,
        )
